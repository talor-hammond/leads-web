...
function getPosts() { // comes with user information, username, user_id, etc.
  return conn('users')
    ...
    .select('posts.id as post_id', 'users.id as user_id', 'users.user_name as username', 'posts.lat as lat', 'posts.long as long', 'posts.title as title', 'posts.topic as topic', 'posts.description as description' )
}
